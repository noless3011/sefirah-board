...
export const updateMyPassword = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = getAuthenticatedUserId(req);
    const { currentPassword, newPassword } = parseBody(UpdatePasswordPayloadSchema, req.body);
    ...
    if (!user.password) {
      throw new AppError('Password change is unavailable for OAuth-only accounts', 400);
    }
...
export const deactivateMyAccount = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = getAuthenticatedUserId(req);
    const { password } = parseBody(DeactivateAccountPayloadSchema, req.body);
    ...
    if (!user.password) {
      throw new AppError('Password confirmation is unavailable for OAuth-only accounts', 400);
    }
    ...
    res
      .status(200)
      .json(ResetPasswordResponseSchema.parse({ message: 'Account deleted successfully' }));
  } catch (error) {
    next(error);
  }
};
