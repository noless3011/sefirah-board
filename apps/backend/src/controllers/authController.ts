...
export const register = async (
    ...
    try {
        const { email, password, fullName } = parseBody(
            RegisterPayloadSchema,
            req.body,
        );
...
export const login = async (
    ...
    try {
        const { email, password, rememberMe } = parseBody(
            LoginPayloadSchema,
            req.body,
        );
...
export const githubOAuth = async (
    ...
    try {
        ...
        const fullName =
            (typeof userResponse.data.name === "string" &&
                userResponse.data.name.trim()) ||
            (typeof userResponse.data.login === "string" &&
                userResponse.data.login.trim()) ||
            "GitHub User";
        const avatarUrl =
            typeof userResponse.data.avatar_url === "string"
...
export const refreshToken = async (
    ...
    try {
        const { refreshToken: providedToken } = parseBody(
            RefreshTokenPayloadSchema,
            req.body,
        );
        ...
        const defaultLifetimeMs =
            DEFAULT_REFRESH_TOKEN_DAYS * 24 * 60 * 60 * 1000;
...
export const forgotPassword = async (
    ...
    try {
        ...
        if (!user) {
            res.status(200).json(
                ResetPasswordResponseSchema.parse({ message: genericMessage }),
            );
            return;
        }
        ...
        logger.info(
            `[Auth] Password reset link for ${normalizedEmail}: ${resetUrl}`,
        );
...
export const resetPassword = async (
    ...
    try {
        const { token, newPassword } = parseBody(
            ResetPasswordPayloadSchema,
            req.body,
        );
