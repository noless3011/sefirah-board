...
const LoginPage = () => {
    ...
    const handleGoogleLogin = async (e: React.MouseEvent) => {
        ...
        setError("");
        try {
            const configRes = await authApi.getAuthConfig();
            const { googleClientId } = configRes.data;
            if (!googleClientId) {
                throw new Error("Google Client ID is not configured on the backend.");
            }
            const nonce = Math.random().toString(36).substring(2);
            const redirectUri = encodeURIComponent(`${window.location.origin}/auth/google/callback`);
            const url = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${googleClientId}&redirect_uri=${redirectUri}&response_type=id_token&scope=openid%20profile%20email&nonce=${nonce}`;
    ...
    const handleGithubLogin = async (e: React.MouseEvent) => {
        ...
        try {
            const configRes = await authApi.getAuthConfig();
            const { githubClientId } = configRes.data;
            if (!githubClientId) {
                throw new Error("GitHub Client ID is not configured on the backend.");
            }
            const url = `https://github.com/login/oauth/authorize?client_id=${githubClientId}&scope=user:email`;
            window.location.href = url;
        } catch (err: any) {
            ...
            <main className="flex-1 flex flex-col items-center justify-center px-4 py-10">
                <div className="mb-8 text-center">
                    ...
                    <p className="mt-2 text-sm text-gray-500">
                        Welcome back to the Ethereal Atelier.
                    </p>
                </div>
                ...
                <div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-100 px-8 py-8">
                    ...
                    <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl bg-gray-50 border border-gray-200 pl-10 pr-4 py-3 text-sm text-gray-800 placeholder-gray-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition"
                                    onChange={(e) =>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full rounded-xl bg-gray-50 border border-gray-200 pl-10 pr-10 py-3 text-sm text-gray-800 placeholder-gray-400 outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition"
                                    onChange={(e) =>
                        ...
                        <button
                            type="submit"
                            className="w-full rounded-full py-3 font-semibold text-white text-sm tracking-wide transition-opacity hover:opacity-90 active:opacity-80"
                            style={{
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <button
                            ...
                            className="flex items-center justify-center gap-2 rounded-xl border border-gray-200 bg-white py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition cursor-pointer"
                        >
                            {/* Google SVG */}
