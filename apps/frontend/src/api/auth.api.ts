...
export const authApi = {
    ...
    loginWithGoogle: (token: string) => {
        return axiosClient.post<AuthResponse>(`${AUTH_URL}/oauth/google`, {
            token,
        });
    },

    loginWithGithub: (code: string) => {
        return axiosClient.post<AuthResponse>(`${AUTH_URL}/oauth/github`, {
            code,
        });
    },

    refreshToken: (data: RefreshTokenRequest) => {
        return axiosClient.post<RefreshTokenResponse>(
